...
"[project]/app/api/get-next-serial/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: unidad, error: unidadError } = await supabase.from("unidades_producto").select("numero_serie").eq("id_producto", PRODUCTO_ID).eq("estado", "disponible").order("numero_serie").limit(1).single();
        ...
        // Obtener stock total
        const { data: producto, error: productoError } = await supabase.from("productos").select("stock_inicial").eq("id", PRODUCTO_ID).single();
        if (productoError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f4e0a37f._.js.map